...
void NodeFactory::writeCuesheetsToJSONObj(QJsonObject &json) const
{
    ...
    foreach (const CuesheetScene *cuesheet, Cupid::Singleton()->getGraphWidget()->getCuesheets()) {
...
void NodeFactory::readCuesheets(const QJsonObject &json, bool ignoreCuesheets)
{
    ...
    for (int i = 0; i < cuesheetsArray.size(); ++i) {
        ...
        if (! ignoreCuesheets) {
            ...
            Cupid::Singleton()->getGraphWidget()->newCuesheet(name);
        }
        readAllNodes(cuesheetObject);
    }
    Cupid::Singleton()->getGraphWidget()->deleteEmptyFirstCuesheet();
